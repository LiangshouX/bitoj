...
4. 运行项目，能成功运行就成功了![image-20231106200926975](https://typora-1308640872.cos.ap-beijing.myqcloud.com/img/image-20231106200926975.png)



## 前端工程化配置

> 这一步我并没有做，因为执行格式化快捷键以后，代码就跑不起来了
...
> Vue Router 路由组件已自动引入，无需再引入: https://routervuejs.org/zh/introduction.html
...
根据 [官方文档](https://arco.design/vue/docs/start) 的描述，在 `main.js` 中引入相关组件，本项目引入后的代码如下：
...
进入官方组件库中选择一个 [导航菜单](https://arco.design/vue/component/menu)，在项目的 `components` 目录下降拷贝的代码作为一个单独的 `GlobalHeader.vue` 组件，在 `BasicLayout.vue` 的相应位置引入 `GlobalHeader` 。

根据路由配置信息，自动生成菜单内容。实现更通用、更自动的菜单配置。
步骤:
